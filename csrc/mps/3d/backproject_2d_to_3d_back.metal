...
kernel void backproject_2d_to_3d_back_kernel(
    ...
    constant Params3D&   params           [[buffer(10)]],
    ...
    threadgroup float local_rot_grad[256][9];    // [thread][matrix_element] for 3x3 matrix
    threadgroup float local_shift_grad[256][2];  // [thread][shift_component]
    
    // Initialize local accumulators
    for (int i = 0; i < 9; i++) {
        local_rot_grad[tid][i] = 0.0;
    }
    ...
    int rot_idx_base = ((rot_b_idx * params.P + p) * 3 * 3);
    ...
    for (int32_t pixel_idx = tid; pixel_idx < total_pixels; pixel_idx += tpg.x) {
        ...
        if (need_rotation_grads) {
            ...
            local_rot_grad[tid][0] += (complex_mul(proj_val, complex_conj(complex_scale(grad_c, sample_c)))).x;  // ∂f/∂R[0][0]
            local_rot_grad[tid][1] += (complex_mul(proj_val, complex_conj(complex_scale(grad_c, sample_r)))).x;  // ∂f/∂R[0][1]
            local_rot_grad[tid][2] += (complex_mul(proj_val, complex_conj(complex_scale(grad_c, sample_d)))).x;  // ∂f/∂R[0][2]
            local_rot_grad[tid][3] += (complex_mul(proj_val, complex_conj(complex_scale(grad_r, sample_c)))).x;  // ∂f/∂R[1][0]
            local_rot_grad[tid][4] += (complex_mul(proj_val, complex_conj(complex_scale(grad_r, sample_r)))).x;  // ∂f/∂R[1][1]
            local_rot_grad[tid][5] += (complex_mul(proj_val, complex_conj(complex_scale(grad_r, sample_d)))).x;  // ∂f/∂R[1][2]
            local_rot_grad[tid][6] += (complex_mul(proj_val, complex_conj(complex_scale(grad_d, sample_c)))).x;  // ∂f/∂R[2][0]
            local_rot_grad[tid][7] += (complex_mul(proj_val, complex_conj(complex_scale(grad_d, sample_r)))).x;  // ∂f/∂R[2][1]
            local_rot_grad[tid][8] += (complex_mul(proj_val, complex_conj(complex_scale(grad_d, sample_d)))).x;  // ∂f/∂R[2][2]
        }
