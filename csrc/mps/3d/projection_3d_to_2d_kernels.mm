...
// Get Metal compute pipeline for forward 3D->2D projection
id<MTLComputePipelineState> get_forward_3d_to_2d_projection_pipeline(id<MTLDevice> device) {
    static id<MTLComputePipelineState> pipeline = nil;
    static id<MTLDevice> cached_device = nil;
    static NSString* shaderSource = nil;
    
    if (!shaderSource) {
        shaderSource = [NSString stringWithUTF8String:PROJECTION_3D_KERNEL_SOURCE];
    }
...
at::Tensor forward_project_3d_to_2d_mps(
  ...
  @autoreleasepool {
    ...
    const auto D            = reconstruction.size(1);  // Depth dimension
    const auto boxsize      = reconstruction.size(2);
    const auto boxsize_half = reconstruction.size(3);
    ...
    id<MTLComputePipelineState> pipeline = get_forward_3d_to_2d_projection_pipeline(device);
    ...
    dispatch_sync(serialQueue, ^(){
      ...
      [encoder setBytes:&params length:sizeof(Params3D) atIndex:4];
...
// Get Metal compute pipeline for backward 3D->2D projection
id<MTLComputePipelineState> get_backward_3d_to_2d_projection_pipeline(id<MTLDevice> device) {
    static id<MTLComputePipelineState> pipeline = nil;
    static id<MTLDevice> cached_device = nil;
    static NSString* shaderSource = nil;
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> backward_project_3d_to_2d_mps(
  ...
  @autoreleasepool {
    ...
    auto grad_reconstruction = at::zeros({B, rec_depth, rec_boxsize, rec_boxsize_half}, grad_projections.options());
    ...
    id<MTLDevice> device = [commandBuffer device];
    id<MTLComputePipelineState> pipeline = get_backward_3d_to_2d_projection_pipeline(device);
    ...
    dispatch_sync(serialQueue, ^(){
      ...
      [encoder setBytes:&params length:sizeof(Params3D) atIndex:7];
