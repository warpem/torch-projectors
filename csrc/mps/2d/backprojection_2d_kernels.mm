...
// Get Metal compute pipeline for forward backprojection
id<MTLComputePipelineState> get_forward_backproject_pipeline(id<MTLDevice> device) {
    static id<MTLComputePipelineState> pipeline = nil;
    static id<MTLDevice> cached_device = nil;
    static NSString* shaderSource = nil;
    
    if (!shaderSource) {
        shaderSource = [NSString stringWithUTF8String:BACKPROJECT_KERNEL_SOURCE];
    }
...
std::tuple<at::Tensor, at::Tensor> backproject_2d_forw_mps(
  ...
  @autoreleasepool {
    ...
    // -------- Output tensors on MPS --------
    auto data_reconstruction = at::zeros({B, rec_boxsize, rec_boxsize_half}, projections.options());
    
    at::Tensor weight_reconstruction;
    bool has_weights = weights.has_value();
    if (has_weights) {
        weight_reconstruction = at::zeros({B, rec_boxsize, rec_boxsize_half}, weights->options());
    } else {
    ...
    id<MTLComputePipelineState> pipeline = get_forward_backproject_pipeline(device);
...
// Get Metal compute pipeline for backward backprojection
id<MTLComputePipelineState> get_backward_backproject_pipeline(id<MTLDevice> device) {
    static id<MTLComputePipelineState> pipeline = nil;
    static id<MTLDevice> cached_device = nil;
    static NSString* shaderSource = nil;
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> backproject_2d_back_mps(
  ...
  @autoreleasepool {
    ...
    id<MTLDevice> device = [commandBuffer device];
    id<MTLComputePipelineState> pipeline = get_backward_backproject_pipeline(device);
