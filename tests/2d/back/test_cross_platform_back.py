"""
Cross-platform compatibility tests for torch-projectors back-projection.

This module tests that CPU, CUDA, and MPS implementations produce identical results
for back-projection across all features including forward/backward passes and gradient computation.
"""
...
def test_cpu_cuda_backproject_identical_comprehensive():
    """
    Comprehensive test that CPU and CUDA back-projection implementations produce identical results
    ...
    # Multiple projection sets with different characteristics
    ...
    rotations_cpu = torch.zeros(num_projection_sets, num_poses, 2, 2, device='cpu')
    ...
    for interpolation in ['linear', 'cubic']:
        ...
        # Create realistic loss function for back-projection
        def compute_realistic_backproject_loss(reconstruction, weight_reconstruction):
            """Compute a realistic loss function for back-projection results"""
            ...
            # 2. Total variation regularization (smoothness) on reconstruction
            rec_real = reconstruction.real
            rec_imag = reconstruction.imag
            
            # Real part TV
            tv_r_h = torch.abs(rec_real[:, 1:, :] - rec_real[:, :-1, :])
            tv_r_w = torch.abs(rec_real[:, :, 1:] - rec_real[:, :, :-1])
            tv_real = torch.mean(tv_r_h) + torch.mean(tv_r_w)
            
            # Imaginary part TV  
            tv_i_h = torch.abs(rec_imag[:, 1:, :] - rec_imag[:, :-1, :])
            tv_i_w = torch.abs(rec_imag[:, :, 1:] - rec_imag[:, :, :-1])
            tv_imag = torch.mean(tv_i_h) + torch.mean(tv_i_w)
            
            # 3. Weight consistency term (weights should be smooth and positive)
        ...
        loss_cpu = compute_realistic_backproject_loss(reconstruction_cpu, weight_reconstruction_cpu)
        loss_cpu.backward()
        ...
        loss_cuda = compute_realistic_backproject_loss(reconstruction_cuda, weight_reconstruction_cuda)
        loss_cuda.backward()
        ...
        print(f"✓ {interpolation} interpolation: All CPU/CUDA back-projection results identical within tolerance")
    
    print(f"\n✓ Comprehensive CPU/CUDA back-projection comparison test passed!")
    print(f"  - {num_projection_sets} projection sets × {num_poses} poses tested")
    print(f"  - Both linear and cubic interpolation verified")
    print(f"  - Rotations, shifts, and weight accumulation tested")
...
def test_cpu_mps_backproject_identical_comprehensive():
    """
    Comprehensive test that CPU and MPS back-projection implementations produce identical results
    across all features: multiple projection sets, poses, shifts, interpolations,
    and full forward/backward passes with realistic loss computation.
    """
    torch.manual_seed(42)
    ...
    for interpolation in ['linear', 'cubic']:
        ...
        loss_mps = compute_realistic_backproject_loss(reconstruction_mps, weight_reconstruction_mps)
        loss_mps.backward()
        ...
        print(f"✓ {interpolation} interpolation: All CPU/MPS back-projection results identical within tolerance")
    
    print(f"\n✓ Comprehensive CPU/MPS back-projection comparison test passed!")
    print(f"  - {num_projection_sets} projection sets × {num_poses} poses tested")
    print(f"  - Both linear and cubic interpolation verified")
    print(f"  - Rotations, shifts, and weight accumulation tested")
    print(f"  - Full forward/backward pass with realistic loss function")
