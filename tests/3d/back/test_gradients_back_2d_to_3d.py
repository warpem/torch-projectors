"""
Gradient computation tests for torch-projectors 2D->3D back-projection.

This module tests gradient computation for 2D->3D back-projection including basic gradcheck,
...
def test_backproject_2d_to_3d_gradcheck_projections_only(device, interpolation):
    """
...
@pytest.mark.parametrize("interpolation", ["linear", "cubic"])
def test_backproject_2d_to_3d_gradient_fourier_components(device, interpolation):
    """
    Debug test for gradient calculation of 2D->3D back-projection Fourier components.
    ...
    # 90 degree rotation matrix around Z-axis (3x3)
    ...
    ref_reconstruction, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    ref_reconstruction_0, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    reconstruction_zero, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    # Use central slice for comparison
    ref_reconstruction_0_real = torch.fft.irfft2(ref_reconstruction_0[0, 0], s=(H, W))
    ...
    ref_reconstruction_0_fourier = ref_reconstruction_0[0, 0]  # Central slice
    ...
    plt.tight_layout()
    plt.savefig(f'test_outputs/3d/back/gradient_debug_fourier_components_2d_to_3d_{interpolation}_{device.type}.png')
    plt.close()
    ...
    # Note: 2D->3D back-projection gradients can be more complex than 2D->2D
...
def test_backproject_2d_to_3d_shift_gradient_verification_parametrized(device, interpolation, shift_values, batch_size, num_poses):
    """
    Parametrized test to verify shift gradient calculation for 2D->3D back-projection across various scenarios.
    """
    torch.manual_seed(42)
    ...
    # 180-degree rotation around Z-axis (same for all batches and poses)
    angle_deg = 180.0
    angle_rad = math.radians(angle_deg)
    rot_180 = create_rotation_matrix_3d_z(torch.tensor(angle_rad, device=device))
    rotations = rot_180.unsqueeze(0).unsqueeze(0).expand(B, P, 3, 3)  # Shape: (B, P, 3, 3)
    
    # Step 2: Generate unshifted back-projection at 180 degrees
    reconstruction_unshifted, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    reconstruction_shifted_our_impl, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    reconstruction_shifted_manual, _ = torch_projectors.backproject_2d_to_3d_forw(
    ...
    print(f"2D->3D Back-projection Shift: {shift_values}, Batch: {batch_size}, Poses: {num_poses}")
    ...
    # Create visualization (show central slice)
    os.makedirs('test_outputs/3d/back', exist_ok=True)
    fig, axes = plt.subplots(2, 3, figsize=(12, 8))
    
    # Show first batch for visualization (central slice)
    # Real parts
    axes[0, 0].imshow(reconstruction_unshifted[0, 0].real.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[0, 0].set_title('Unshifted (Real, z=0)')
    axes[0, 0].axis('off')
    
    axes[0, 1].imshow(reconstruction_shifted_our_impl[0, 0].real.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[0, 1].set_title('Shifted - Our Impl (Real, z=0)')
    axes[0, 1].axis('off')
    
    axes[0, 2].imshow(reconstruction_shifted_manual[0, 0].real.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[0, 2].set_title('Shifted - Manual (Real, z=0)')
    axes[0, 2].axis('off')
    
    # Imaginary parts
    axes[1, 0].imshow(reconstruction_unshifted[0, 0].imag.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[1, 0].set_title('Unshifted (Imag, z=0)')
    axes[1, 0].axis('off')
    
    axes[1, 1].imshow(reconstruction_shifted_our_impl[0, 0].imag.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[1, 1].set_title('Shifted - Our Impl (Imag, z=0)')
    axes[1, 1].axis('off')
    
    axes[1, 2].imshow(reconstruction_shifted_manual[0, 0].imag.detach().cpu().numpy(), cmap='viridis', origin='lower')
    axes[1, 2].set_title('Shifted - Manual (Imag, z=0)')
    axes[1, 2].axis('off')
    
    plt.tight_layout()
    plt.savefig(f'test_outputs/3d/back/backproject_2d_to_3d_shift_gradient_verification_{shift_values[0]}_{shift_values[1]}_B{batch_size}_P{num_poses}_{interpolation}_{device.type}.png')
    plt.close()
    
    print(f"✅ 2D->3D back-projection shift gradient test passed: {shift_values} (B={batch_size}, P={num_poses})")


@pytest.mark.parametrize("interpolation", ["linear", "cubic"])
def test_backproject_2d_to_3d_rotation_gradients_comprehensive(device, interpolation):
    """
    Comprehensive test of 3D rotation gradients for 2D->3D back-projection using finite difference verification.
    This test validates that our analytical rotation gradients match numerical derivatives.
    """

    torch.manual_seed(42)

    print(f"\n🔄 Testing 2D->3D back-projection rotation gradients comprehensively ({interpolation})...")
    
    # Test 1: Finite Difference Verification (most important)
    print("  1️⃣ Finite difference verification...")
    _test_backproject_2d_to_3d_rotation_finite_difference_accuracy(device, interpolation)
    ...
    for i, (target_angle, test_offset) in enumerate(test_cases):
        ...
        target_reconstruction, _ = torch_projectors.backproject_2d_to_3d_forw(projections.detach(), rotations=target_rot.detach(), interpolation=interpolation)
        ...
        pred_reconstruction, _ = torch_projectors.backproject_2d_to_3d_forw(projections.detach(), rotations=test_rot, interpolation=interpolation)
    ...
    _test_backproject_2d_to_3d_rotation_optimization_convergence(device, interpolation)
    
    print("✅ All 2D->3D back-projection rotation gradient tests passed!")


def _test_backproject_2d_to_3d_rotation_finite_difference_accuracy(device, interpolation):
    """Test that analytical gradients match finite differences for 2D->3D back-projection"""
    ...
    reconstruction_plus, _ = torch_projectors.backproject_2d_to_3d_forw(projections.detach(), rotations=rot_plus, interpolation=interpolation)
    reconstruction_minus, _ = torch_projectors.backproject_2d_to_3d_forw(projections.detach(), rotations=rot_minus, interpolation=interpolation)
...
def _test_backproject_2d_to_3d_rotation_optimization_convergence(device, interpolation):
    """Test that optimization converges to correct angle for 2D->3D back-projection"""
    ...
    for target_angle in target_angles:
        ...
        target_rot = create_rotation_matrix_3d_z(torch.tensor(target_angle, device=device)).unsqueeze(0).unsqueeze(0)
        target_reconstruction, _ = torch_projectors.backproject_2d_to_3d_forw(projections, rotations=target_rot, interpolation=interpolation)
...
@pytest.mark.parametrize("interpolation", ["linear", "cubic"])
def test_backproject_2d_to_3d_weight_gradients(device, interpolation):
    """
    Tests gradient computation with respect to weights in 2D->3D back-projection.
    """
    torch.manual_seed(42)
    ...
    # Identity and 45-degree rotations around Z-axis (3x3 matrices)
    rotations = torch.zeros(B, P, 3, 3, dtype=torch.float32, device=device)
    rotations[0, 0] = torch.eye(3, device=device)
    angle = math.pi / 4  # 45 degrees
    rotations[0, 1] = create_rotation_matrix_3d_z(torch.tensor(angle, device=device))
    ...
    reconstruction_plus, weight_reconstruction_plus = torch_projectors.backproject_2d_to_3d_forw(
    ...
    reconstruction_minus, weight_reconstruction_minus = torch_projectors.backproject_2d_to_3d_forw(
