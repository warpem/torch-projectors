...
\section{Discussion}
The 1–2 order of magnitude performance improvements demonstrated here enable previously computationally infeasible machine learning applications in cryo-EM, where projection operations must be performed thousands of times during gradient-based optimization. The benchmarks reveal that cubic interpolation without oversampling becomes advantageous when reference volumes cannot be pre-padded offline, offering approximately 4× speedup over on-the-fly oversampling approaches – a common scenario in memory-constrained neural networks with dynamically generated intermediate representations. Future extensions of this library could include 3D→3D projection operators for subtomogram averaging and general volume rotation with accurate interpolation, as well as real-space projection operators that would broaden applicability beyond traditional Fourier-space cryo-EM workflows.
