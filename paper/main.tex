...
\section{Results}
Performance benchmarks were conducted on three representative platforms: NVIDIA H100 CUDA, Apple M4 CPU, and Apple M4 MPS. Throughput measurements in thousands of projections per second were obtained across systematic parameter sweeps including box sizes (32, 128, 256 pixels), batch dimensions (1, 8), pose counts (8, 128, 2048), and interpolation methods (linear, cubic). Benchmarking each test case included 3 warm-up runs, followed by 15 timed runs, for which the median throughput is reported.

\subsection{Performance Benchmarks}

Because computation is parallelized per-projection (1 thread per projection on CPU, 1 thread block per projection on MPS and CUDA), projection throughput exhibits strong scaling with batch size and pose count across all platforms (Tables~\ref{tab:project-2d-performance}--\ref{tab:backproject-2d-to-3d-performance}). Overall, there is a significant performance gap between the platforms, with CUDA implementations achieving the highest throughput, followed by MPS and CPU. The difference between CPU and CUDA is ca. 2 orders of magnitude, owing to the much higher computational throughput and memory bandwidth of the H100 chip.

Cubic interpolation requires 4x more samples than linear interpolation in 2D, and 8x more in 3D. In practice, the penalty isn't quite as large, possibly because each group of samples in the fastest dimension is co-located in memory, and the difference between reading 2 and 4 samples is smaller. Overall, scattered memory access patterns are the biggest performance bottleneck in these algorithms. This is especially pronounced on GPUs, which are optimized for large, coalesced reads and writes.
...
Direct comparison with the existing \texttt{torch-fourier-slice} library demonstrates substantial performance improvements (Tables~\ref{tab:project-3d-to-2d-comparison}, \ref{tab:backproject-2d-to-3d-comparison}). \texttt{torch-projectors} consistently outperforms \texttt{torch-fourier-slice} by 1-2 orders of magnitude across all tested configurations. The new operators also have a significantly lower memory footprint because it calculates everything in a single step and doesn't keep intermediate tensors.

For \texttt{torch-fourier-slice}, most testing scenarios for forward projection could only be tested in the forward pass, as memory consumption in the backward pass exceeded the test system's memory. One particularly memory-intensive backward projection scenario could not be tested in either pass.

The CUDA backend is especially sensitive to replacing a long chain of kernel invocations with a single custom kernel, as it reduces GPU driver overhead and dramatically improves memory locality in an IO-starved algorithm. The performance gap is less pronounced on CPU and MPS, where the relative overhead of multiple kernel calls is smaller, and the ratio between ALUs and memory bandwidth is lower.
...
3 scenarios were benchmarked to provide guidance on when to use which interpolation scheme. 1: 4096 128\,px 2D references projected without padding using linear or cubic interpolation, 1 pose per reference. 2: With offline 2x padding, i.e. preparing 128\,px projections from 256\,px references. 3: Same as 2, but the padding process (resulting in additional IFFT, padding, and FFT operations) was included in the timed code. The last scenario is meant to be representative of use cases where references are an intermediate representation in a longer processing chain and can't be pre-padded offline. In all scenarios, forward and backward passes through the operator chain were timed together.

While sampling on an oversampled grid incurs a noticeable performance penalty (ca. 10\% on M4 CPU, 25\% on M4 MPS, 35\% on H100 CUDA), likely due to increased cache misses when accessing the larger amounts of reference data. This should be less of a problem if the number of references is reduced. However, using cubic interpolation without oversampling is still significantly slower, with the notable exception of the MPS platform. The equation changes if the oversampling needs to be achieved by padding the references on-the-fly, which adds expensive Fourier transforms to the process. Cubic interpolation without oversampling is ca. 4x faster than linear interpolation with on-the-fly 2x padding in this scenario across all platforms.
